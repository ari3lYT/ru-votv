...
* **`ZZ_UISettingsRuPatch_P.pak`** — перевод вшитых текстов меню настроек (`ui_settings.uasset/.uexp`), которые не покрываются обычным `locres`. Текущий файл пересобран из `ui_settings` актуальной `a09k`, старый бинарный override больше не используется.
...
* Часть строк в меню настроек была вшита прямо в `ui_settings`-ассет, поэтому для них собран отдельный UI override-пак из текущего ассета игры.
* `translations/UI/ui_settings_map.json` покрывает все непустые исходные строки текущего `ui_settings`: `311` переводимых текстовых свойств из `315`; оставшиеся свойства пустые/служебные.
...
Если хочешь ещё и вытаскивать исходные `.locres` из игры сам, а не только пересобирать уже готовый CSV, тогда дополнительно пригодится любой распаковщик `.pak`, например `repak`, `u4pak` или `FModel`. Для пересборки `ZZ_UISettingsRuPatch_P.pak` нужен `repak` и Rust/Cargo: скрипт патчит текущий `ui_settings.uasset/.uexp` через маленький helper в `tools/ui_settings_patcher`.
...
### Пересборка UISettings-пака

`ZZ_UISettingsRuPatch_P.pak` нельзя безопасно переносить между старыми и новыми билдами как готовый бинарник: `ui_settings.uasset/.uexp` меняется вместе с игрой. Для текущей версии пересобери его из оригинального `VotV-WindowsNoEditor.pak`:
...
Скрипт извлечёт текущий `ui_settings`, применит `translations/UI/ui_settings_map.json` и упакует новый pak в формате `V11` с mount-point `../../../`.
